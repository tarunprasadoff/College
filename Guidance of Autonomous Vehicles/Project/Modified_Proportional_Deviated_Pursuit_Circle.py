...
def alphaCompute(prevCoordLeader, coordLeader, coordFollower):
    ...
    if alpha > upperAlphaLimit:
        return upperAlphaLimit
    elif alpha < lowerAlphaLimit:
        return lowerAlphaLimit
    else:
        return alpha
  ...
  for i in range(len(followers)): # Iterating for each Follower Robot
    ...
    if (i==0):
        if (step==0):
            ...
            alpha = alphaCompute(trajectoryCoords[step-1],trajectoryCoords[step],followers[i].coord)
        ...
        if (step==0):
            ...
            alpha = alphaCompute(followers[i-1].trajectory[-3],followers[i-1].prevCoord,followers[i].coord)
...
# ani.save(f"Modified_Proportional_Deviated_Pursuit_Circle.mp4",writer=writer)
