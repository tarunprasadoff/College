...
class Follower:
  def __init__(self, initCoord):
    ...
    self.deviation = None # Remembering the previous deviation for PD coontrol
...
# Defining the function to compute Dynamic alpha based on deviation error.

deviationDf = pd.DataFrame(columns=["theta","followerNo","deviation"])

def alphaCompute(prevCoordLeader, coordLeader, coordFollower, prevDeviation, thetaCurr, deviationDf, followerNo):
    kp = -.4 # Proportionally Constant
    kd = 0
    upperAlphaLimit = (math.pi/180)*10
    lowerAlphaLimit = -upperAlphaLimit
    deviation = (getOrientationAngle(prevCoordLeader, coordLeader)-getOrientationAngle(coordFollower, coordLeader))%(2*math.pi)
    if deviation > math.pi:
        deviation = -(2*math.pi-deviation)
    if prevDeviation == None:
      diffDeviation = 0
    else:
      diffDeviation = deviation - prevDeviation
    deviationDf = deviationDf.append({"theta":((thetaCurr*180)/math.pi),"followerNo":followerNo,"deviation":deviation},ignore_index=True)
    alpha = kp*deviation + kd*diffDeviation
    if alpha > upperAlphaLimit:
        return upperAlphaLimit, deviation, deviationDf
    elif alpha < lowerAlphaLimit:
        return lowerAlphaLimit, deviation, deviationDf
    else:
        return alpha, deviation, deviationDf
...
amp = n*dist*2.5 # Amplitude of Sine Curve

timePeriods = 1 # Number of Time Periods of Periodic Motion

xStretch = 20 # Stretching Coefficient of xy scale

# Computing the Leader Trajectory Coordinates
trajectoryCoords = [([((xStretch*i*math.pi)/steps),(amp*math.sin((2*i*math.pi)/steps))]) for i in range(timePeriods*steps+1)]
...
trajectoryVelocityOrientations = [getOrientationAngle(trajectoryCoords[i+1],trajectoryCoords[i])%(2*math.pi) for i in range(len(trajectoryCoords)-1)]
  ...
  for i in range(len(followers)): # Iterating for each Follower Robot
    ...
    if (i==0):
        if (step==0):
          ...
          alpha, followers[i].deviation, deviationDf = alphaCompute(trajectoryCoords[step-1],trajectoryCoords[step],followers[i].coord,followers[i].deviation, ((2*step*math.pi)/steps), deviationDf, i)
        vTemp, thTemp = dpCompute((-3*(math.pi/180)), trajectoryCoords[step], followers[i].coord, trajectoryVelocities[step], trajectoryVelocityOrientations[step])
    else:
        if (step==0):
          alpha = 0
        else:
          alpha, followers[i].deviation, deviationDf = alphaCompute(followers[i-1].trajectory[-3],followers[i-1].prevCoord,followers[i].coord,followers[i].deviation, ((2*step*math.pi)/steps), deviationDf, i)
        vTemp, thTemp = dpCompute((-3*(math.pi/180)), followers[i-1].prevCoord, followers[i].coord, followers[i-1].velocity, followers[i-1].theta)
...
def animate(i):
  ...
  plt.title(f"Modified Proportional Deviated Pursuit Algorithm for a Robotic Convoy")

  plt.xlim(-.25*amp,1.1*xStretch*timePeriods*math.pi)
...
# ani.save(f"Modified_Proportional_Deviated_Pursuit_Sine.mp4",writer=writer)

deviationDf.to_csv("DP_Sine_Deviation_Alpha_-3Degrees.csv",index=False)
