...
dist = 80 # Initial distance between each convoy robot
...
expCoeff = 1 # Exponential Growth Rate of Spiral Radius

# Computing the Leader Trajectory Coordinates
trajectoryCoords = [([(i**expCoeff)*math.cos(((2*i*math.pi)/steps)+math.pi),(i**expCoeff)*math.sin(((2*i*math.pi)/steps)+math.pi)]) for i in range(timePeriods*steps+1)]
...
def animate(i):
  ...
  plt.xlim(-1.1*((timePeriods*steps)**expCoeff),1.1*((timePeriods*steps)**expCoeff))
  plt.ylim(-1.1*((timePeriods*steps)**expCoeff),1.1*((timePeriods*steps)**expCoeff))
